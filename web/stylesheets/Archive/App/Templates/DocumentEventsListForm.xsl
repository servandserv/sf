...
<xsl:template match="evs:Events">
    ...
    <div>
        <ul class="collection">
            <xsl:for-each select="evs:Event">
                <li class="collection-item">
                    <div>
                        <xsl:value-of select="evs:name" />
                        <a href="javascript:void(0);" class="secondary-content document-event-link-remove-btn red-text text-darken-2" data-id="{link:Link/link:ID}">
