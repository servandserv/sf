...
<xsl:template match="docs:Document">
    <div>
    <form id="document-events-add-form" action="#" method="post" class="row">
        <input type="hidden" name="document-id" value="{docs:ID}" />
        <div class="input-field col s9">
            <select id="document-event-id" name="document-event-id" class="browser-default">
			</select>
		</div>
        <div class="input-field col s3">
             <a class="btn-floating waves-effect waves-light teal darken-1 document-event-add-btn"><i class="material-icons">add</i></a>
        </div>
    </form>
    </div>
</xsl:template>
